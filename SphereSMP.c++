...
vvec eval (const int n, vvec men, vvec women) {
	...
	while(freeMen.size() > 0){
		...
		if (DEBUG) cerr << "free man : " << man << endl;
		if (DEBUG) cerr << "free size: " << freeMen.size() << endl;

		freeMen.pop();		// O(1)
		int wIdx = men[man][n+1];
		int woman = men[man][wIdx];		// next woman to propose to
		++men[man][n+1];				// set next proposal index
		...
		if(women[woman][0] == 0){	// free woman
			...
			if(women[woman][man] < women[woman][fiance] ){
				...
				freeMen.push(man);		// man found no willing woman, put back in free pool
		}
	}
	...
	for(int i = 0; i < n; ++i){
		...
		if (DEBUG) cerr << i+1 << " did not propose " << men[i+1][n+1] << endl;
		//assert(men[i+1][n+1] == n+1);
	}
